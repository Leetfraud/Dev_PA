...
const repos = [
  { name: "linux", desc: "Linux kernel source tree", stars: 18200, forks: 5600, watchers: 832, lang: "C", langColor: "#555599", activity: 99, rank: 1 },
  { name: "git", desc: "Fast, scalable, distributed revision control system", stars: 4800, forks: 1200, watchers: 304, lang: "C", langColor: "#555599", activity: 88, rank: 2 },
  { name: "subsurface", desc: "Dive log program for Linux, Mac and Windows", stars: 710, forks: 230, watchers: 48, lang: "C++", langColor: "#F59E0B", activity: 72, rank: 3 },
  { name: "uemacs", desc: "Micro Emacs git repository", stars: 280, forks: 69, watchers: 18, lang: "C", langColor: "#555599", activity: 55, rank: 4 },
  { name: "libdc1394", desc: "Firewire camera access library", stars: 64, forks: 22, watchers: 8, lang: "C", langColor: "#555599", activity: 30, rank: 5 },
];

const weeklyData = [28, 42, 35, 60, 55, 80, 74, 90, 65, 78, 88, 95];
...
export default function RepoAnalytics() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      <div>
        <h1 className="text-slate-800 font-bold text-xl">Repository Analytics</h1>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="divide-y divide-slate-50">
          {repos.map((repo) => (
            <div key={repo.name} className="flex items-center gap-4 px-5 py-3.5 hover:bg-slate-50/70 transition-colors group">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    ...
                    style={{ color: repo.langColor, background: repo.langColor + "18" }}
                  >
                    {repo.lang}
                  </span>
                </div>
                <p className="text-slate-400 text-xs mt-0.5 truncate">{repo.desc}</p>
              </div>
              <div className="flex items-center gap-5 text-xs text-slate-500 shrink-0">
                <span className="flex items-center gap-1">
                  <Star size={11} className="text-amber-400" /> {repo.stars >= 1000 ? `${(repo.stars / 1000).toFixed(1)}k` : repo.stars}
                </span>
                <span className="flex items-center gap-1">
                  <GitFork size={11} className="text-cyan-400" /> {repo.forks}
                </span>
