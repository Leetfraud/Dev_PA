...
export default function Dashboard({ user, onNavigate, screens }) {
  ...
  useEffect(() => {
    ...
    chartInstance.current = new Chart(chartRef.current, {
      type: "doughnut",
      data: {
        labels: user.languages.map((l) => l.name),
        datasets: [
          {
            data: user.languages.map((l) => l.pct),
            backgroundColor: user.languages.map((l) => l.color),
            borderWidth: 0,
            hoverOffset: 6,
          },
        ],
  ...
  const stats = [
    { label: "Repositories", value: user.public_repos, icon: BookOpen, color: "#0b8868", sub: "public repos" },
          ...
          <div className="flex items-center gap-6">
            <div className="relative w-32 h-32 shrink-0">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-slate-700 font-bold text-sm">{user.languages.length}<br /><span className="text-slate-400 text-xs font-normal">langs</span></span>
              </div>
            </div>
            <div className="flex-1 space-y-2.5">
              {user.languages.map((l) => <LangBar key={l.name} {...l} />)}
            </div>
          </div>
        </div>
